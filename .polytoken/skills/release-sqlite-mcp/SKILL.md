...
GitHub's API shapes matter during recovery: draft releases are invisible to by-tag lookups and to `gh release view`/`gh release edit` until published, so list drafts with `gh api --paginate repos/{owner}/{repo}/releases` and match on the raw REST field `tag_name` (snake_case; the camelCase `tagName` spelling belongs only to the separate `gh release view --json` output shape and never matches a REST payload); stored release bodies carry one canonical trailing newline, so compare notes with that line ending ignored. The xtask publisher implements both behaviors; if it reports `manual recovery required`, reproduce its checks (tag target, exact SHA, exact asset names, digest-verified assets, notes equality) before any manual `draft=false` publication.
